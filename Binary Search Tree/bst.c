...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	//tree is at the end of the leaf and can be added
	if (cur == 0) {
	...
	//value is less than current value, and can go left
	if (compare(val, cur->val) <= 0)
		cur->left = _addNode(cur->left, val);
	//value is greater than current value, and can go right
	else
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	//check to see if the tree has 0 values
	if (tree->cnt <= 0)
		return 0; //empty so return 

	//set curr(ent) node to the root to move downwards
	struct Node *curr = tree->root;

	//loop until the next value down the tree is 0 (can be added)
	while (curr != 0) {
		...
		//value is equal to the value at the node, and is found
		else
			return 1;
	}
	//while loop ended and value not found
	return 0;
}
...
struct Node *_removeLeftMost(struct Node *cur)
{
	...
	//if current left is 0, we are furthest left
	if (cur->left == 0) {
		struct Node *returnNode = cur->right;
		free(cur);

		//return right node to replace the one we free'd
		return returnNode;
	}
	else
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	//if compare returns -1, then move down left
	if (compare(val, cur->val) == -1) {
		cur->left = _removeNode(cur->left, val);
	}
	//if compare returns 1, move to the right
	else if (compare(val, cur->val) == 1) {
		cur->right = _removeNode(cur->right, val);
	}
	//neither have happened, need to determine a node to remove
	else {
		//no nodes to traverse
		if (cur->left == 0 && cur->right == 0)
			return NULL;
		//nothing to the left, so return the right
		else if (cur->left == 0) {
			struct Node *returnNode = cur->right;
			free(cur);
			return returnNode;
		}
		//nothing to the right, return the left
		else if (cur->right == 0) {
			struct Node *returnNode = cur->left;
			free(cur);
			return returnNode;
		}
		//can still traverse so keep moving down tree
		else {
